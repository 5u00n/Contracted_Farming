                                                        ...
                                                        for (int i = 0; i < plot_ids.size(); i++) {
                                                            ...
                                                            if (snapshotU.getKey().equals("manager")) {
                                                                ...
                                                                if (snapshotUsers.child("my_farmers").child(auth.getUid()).exists()) {
                                                                    ...
                                                                    FarmerPlantModel model=new FarmerPlantModel(farmer_plant_id,plot_ids.get(i),plant_id,snapshot.child("farmer").child(auth.getUid()).child("userUID").getValue().toString(),snapshotUsers.child("userUID").getValue().toString(),plot_names.get(i),plant_name,snapshot.child("farmer").child(auth.getUid()).child("username").getValue().toString(),snapshotUsers.child("username").getValue().toString(),plant_img_url,plot_urls.get(i),snapshot.child("farmer").child(auth.getUid()).child("img_url").getValue().toString(),snapshotUsers.child("img_url").getValue().toString(),"waiting","Waiting","Waiting",time_stamp,"waiting","waiting",0);
